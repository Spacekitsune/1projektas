jQuery(document).ready(function($) {
    'use strict';
    var this_obj = web_designer_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.web-designer-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message web-designer-plugin-install');
                button.addClass('button-primary web-designer-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.web-designer-recommended-plugins .web-designer-activation-note').css('display','block');
    ...
    $(document).on('click', '.web-designer-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .web-designer-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .web-designer-recommended-plugins').css('display','none');
    });
